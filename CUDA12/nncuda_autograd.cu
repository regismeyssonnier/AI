...
// grad_W += X^T * grad_Y
__global__ void batch_matmul_backward_W_kernel(const float* X, const float* grad_Y, float* grad_W,
    int batch_size, int nin, int nout)
{
    int row = blockIdx.y * blockDim.y + threadIdx.y; // sortie
    int col = blockIdx.x * blockDim.x + threadIdx.x; // entrée

    if (row < nout && col < nin) {
        float sum = 0.0f;
        for (int b = 0; b < batch_size; ++b)
            sum += X[b * nin + col] * grad_Y[b * nout + row];
        atomicAdd(&grad_W[row * nin + col], sum); // éviter les conflits
    }
}

// grad_B += sum over batch of grad_Y
__global__ void batch_matmul_backward_B_kernel(const float* grad_Y, float* grad_B,
...
// grad_X += grad_Y * W^T
__global__ void batch_matmul_backward_X_kernel(const float* grad_Y, const float* W, float* grad_X,
    int batch_size, int nin, int nout)
{
    ...
    std::shared_ptr<UnityGPU> forward_batch(std::shared_ptr<UnityGPU> X, int batch_size) {
        ...
        if (!NOGRAD) {
            ...
            Y->_backward = [this, X, Y, batch_size]() {
                ...
                batch_matmul_backward_W_kernel << <blocks_W, threads >> > (X->data, Y->grad, W->grad, batch_size, nin, nout);

                int threads_B = 256;
                int blocks_B = (nout + threads_B - 1) / threads_B;
                batch_matmul_backward_B_kernel << <blocks_B, threads_B >> > (Y->grad, B->grad, batch_size, nout);

                dim3 blocks_X((nin + threads.x - 1) / threads.x, (batch_size + threads.y - 1) / threads.y);
                batch_matmul_backward_X_kernel << <blocks_X, threads >> > (Y->grad, W->data, X->grad, batch_size, nin, nout);
            };
        }
...
__global__ void mse_loss_kernel(const float* pred, const float* target, float* grad, int n) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < n) {
        grad[idx] = 2.0f * (pred[idx] - target[idx]);
    }
}
...
void Training_GPU_Fast()
{
    ...
    const int nin = 2;
    std::vector<int> nouts = { 4, 1 };
    std::vector<int> nonlin = { 1, 2 }; // Sigmoid
